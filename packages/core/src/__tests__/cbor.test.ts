...
import { encode, CborTag, encodeItem } from "../cbor.js";
...
describe("encodeItem — primitives", () => {
  ...
  it("encodes a Map with integer keys", () => {
    ...
    const decoded = decode(encode(m), { useMaps: true }) as Map<number, unknown>;
...
describe("CborTag", () => {
  it("encodes tag + value — 0xd8 0x25 header for tag 37", () => {
...
describe("encode — round-trip", () => {
  it("round-trips a nested map through cborg", () => {
    ...
    const decoded = decode(encode(outer), { useMaps: true }) as Map<number, unknown>;
